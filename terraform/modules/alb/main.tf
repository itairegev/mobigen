...
# Target Group for Generator Service
resource "aws_lb_target_group" "generator" {
  name        = "${var.name_prefix}-gen-tg"
  port        = 4000
  protocol    = "HTTP"
  vpc_id      = var.vpc_id
  target_type = "ip"
  ...
  tags = merge(var.common_tags, {
    Name = "${var.name_prefix}-generator-tg"
  })
}
...
# Listener Rule for Generator API (/api/generate, /socket.io)
resource "aws_lb_listener_rule" "generator" {
  listener_arn = var.certificate_arn != "" ? aws_lb_listener.https[0].arn : aws_lb_listener.http.arn
  priority     = 100

  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.generator.arn
  }

  condition {
    path_pattern {
      values = ["/api/generate*", "/socket.io/*"]
    }
  }
}
